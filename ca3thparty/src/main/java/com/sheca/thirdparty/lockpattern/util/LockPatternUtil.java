package com.sheca.thirdparty.lockpattern.util;
...
public class LockPatternUtil {
    ...
    private static String SPNAME = "LockPatternSP";
    private static String KEYNAME = "sppsd";
    ...
    public static String getPatternValue(List<LockPatternView.Cell> pattern){
        if (null == pattern) {
            return null;
        } else {
            byte[] bytes2 = patternToHash(pattern);
    ...
    public static void savePattern(Context context,String value){
        SharedPreferences sp = context.getSharedPreferences(SPNAME, Activity.MODE_PRIVATE);
        SharedPreferences.Editor editor = sp.edit();
        editor.putString(KEYNAME+ACTName, value);
        editor.commit();
        
        SharedPreferences sharedPrefs = context.getSharedPreferences("settings", Context.MODE_PRIVATE);
        editor = sharedPrefs.edit();
        editor.putBoolean("SETTINGS_GESTURE_OPENED"+ACTName, true);	
        editor.commit();
    }

    public static boolean checkLocalPattern(Context context,List<LockPatternView.Cell> pattern){
        SharedPreferences sp = context.getSharedPreferences(SPNAME, Activity.MODE_PRIVATE);
        String value = sp.getString(KEYNAME+ACTName, "");

        return value.equals(getPatternValue(pattern));
    }
