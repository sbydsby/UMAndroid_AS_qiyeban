	...
	public static   String getRSASign(final Cert cert) {
		...
		try {	
		   if("1".equals(strMsgWrapper)){
			  strSignDate = PKIUtil.sign(Base64.decode(message),cert.getKeystore(), strCertPwd);	
		   }else if("0".equals(strMsgWrapper)){				
			  strSignDate = PKIUtil.sign(Base64.decode(message),cert.getKeystore(), strCertPwd);	
           }else{
        	 strSignDate = PKIUtil.sign(message.getBytes(CommonConst.SIGN_STR_CODE),cert.getKeystore(), strCertPwd);
           }	
