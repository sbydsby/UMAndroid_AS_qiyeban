...
import com.sheca.umandroid.companyCert.album.LoadingDialog;
import com.sheca.umandroid.companyCert.album.camera.lisenter.CaptureLisenter;
import com.sheca.umandroid.companyCert.album.camera.lisenter.ErrorLisenter;
import com.sheca.umandroid.companyCert.album.camera.lisenter.FirstFoucsLisenter;
import com.sheca.umandroid.companyCert.album.camera.lisenter.JCameraLisenter;
import com.sheca.umandroid.companyCert.album.camera.lisenter.ReturnLisenter;
import com.sheca.umandroid.companyCert.album.camera.lisenter.TypeLisenter;
...
public class JCameraView extends FrameLayout implements CameraInterface.CamOpenOverCallback, SurfaceHolder.Callback {
    ...
    private CircleSealView mCircleView;
...
//        });
        ...
        mCircleView=new CircleSealView(mContext);
        LayoutParams circle_param = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
        circle_param.gravity = Gravity.CENTER;
        mCircleView.setLayoutParams(circle_param);

        mTextView=new TextView(mContext);
        LayoutParams text_param = new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
        text_param.gravity = Gravity.TOP|Gravity.CENTER_HORIZONTAL;
        mTextView.setText(getResources().getString(R.string.take_photo_tip));
        mTextView.setTextColor(getResources().getColor(R.color.white));
        mTextView.setVisibility(View.VISIBLE);
        mTextView.setGravity(Gravity.CENTER);
        text_param.setMargins(0,400,0,0);
        mTextView.setLayoutParams(text_param);
