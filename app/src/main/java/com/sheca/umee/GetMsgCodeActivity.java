...
import com.sheca.umee.layout.VerificationCodeView;
...
public class GetMsgCodeActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (extras != null){ 
			...
			//Toast.makeText(GetMsgCodeActivity.this, strMobile,Toast.LENGTH_SHORT).show();
		}
		
		mAccountDao = new AccountDao(GetMsgCodeActivity.this);
		mCertDao = new CertDao(GetMsgCodeActivity.this);
		mAppInfoDao = new AppInfoDao(GetMsgCodeActivity.this);
	    ...
	    verificationcodeview.setOnCodeFinishListener(new VerificationCodeView.OnCodeFinishListener() {
	            ...
	            public void onComplete(final String content) {
	                ...
	                //短信验证码登录
	                final Handler handler = new Handler(GetMsgCodeActivity.this.getMainLooper());
	        						 ...
	        						 @Override
	        							public void run(){
	        							 try{
	        								 ...
	        								 Toast.makeText(GetMsgCodeActivity.this, mError,Toast.LENGTH_SHORT).show(); 
	        							 }
	        						 }
	        	            	});
	        	            }
	...
	private  void getValidationCode(String phone){
		...
		try {
			//异步调用UMSP服务：获取短信验证码
			String timeout = GetMsgCodeActivity.this.getString(R.string.WebService_Timeout);				
			String urlPath = GetMsgCodeActivity.this.getString(R.string.UMSP_Service_GetValidationCode);
	...
	public boolean  userLoginByValidationCode(String mobile,String code)  throws Exception{
			...
			//异步调用UMSP服务：用户登录
			String timeout = GetMsgCodeActivity.this.getString(R.string.WebService_Timeout);				
			String urlPath = GetMsgCodeActivity.this.getString(R.string.UMSP_Service_LoginByDynamicCode);
		    ...
		    if (resultStr.equals("0")) {
			    ...
			    Intent intent = new Intent(GetMsgCodeActivity.this, AccountActivity.class);
			    ...
			    Intent intent = new Intent(GetMsgCodeActivity.this, PasswordActivity.class);
	...
	private  int  getPersonalInfo(){
		...
		try {
			String timeout = GetMsgCodeActivity.this.getString(R.string.WebService_Timeout);				
			String urlPath = GetMsgCodeActivity.this.getString(R.string.UMSP_Service_GetPersonalInfo);
	...
	private  void   getAllAppInfos(String actNo)  throws Exception{
		...
		try {
			...
			String timeout = GetMsgCodeActivity.this.getString(R.string.WebService_Timeout);				
			String urlPath = GetMsgCodeActivity.this.getString(R.string.UMSP_Service_GetAllAppInfos);										
			Map<String,String> postParams = new HashMap<String,String>();	
			//responseStr = WebClientUtil.httpPost(urlPath, postParams, Integer.parseInt(timeout));
