...
public class CertRenameActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_cert_rename);
		
		getWindow().setFeatureInt(Window.FEATURE_CUSTOM_TITLE, R.layout.title);
		((TextView) findViewById(R.id.header_text)).setText("设置证书别名");
		...
		cancelScanButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				CertRenameActivity.this.finish();
			}
		});
	...
	private  void  doRenameCert(final int  certId){
		...
		if (sCertName != null && !"".equals(sCertName)) {
			...
			Toast.makeText(CertRenameActivity.this, "证书别名修改成功",Toast.LENGTH_SHORT).show();  
		    Intent intent = new Intent(CertRenameActivity.this, MainActivity.class);	
			startActivity(intent);	
		} else {
			Toast.makeText(CertRenameActivity.this,"证书别名不能为空", Toast.LENGTH_SHORT).show();
		}
