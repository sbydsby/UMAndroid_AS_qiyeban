...
import com.sheca.zhongmei.presenter.ExcecuteObservable;
import com.sheca.zhongmei.util.CommUtil;
import com.sheca.zhongmei.util.CommonConst;
import com.sheca.zhongmei.util.IFAACommonUtil;
import com.sheca.zhongmei.util.IFAAConstant;
...
public class SettingIFAAFaceTypeActivity extends Activity {
    ...
    // 认证类型
    private IFAAAuthTypeEnum ifaaAuthType = IFAAAuthTypeEnum.AUTHTYPE_FACE;
    // 用户需要提供给IFAA的基本信息类
...
//            FingerprintBroadcastUtil.sendIdentifyStatusChangeMessage(SettingIFAAFaceTypeActivity.this, status);
//        }
//
//        @Override
...
//                  IFAAFingerprintOpenAPI.getInstance().sendIFAARegResponeAsyn(SettingIFAAFaceTypeActivity.this,data, secData, callback);
//                //IFAAFingerprintOpenAPI.getInstance().sendRegResponeAsyn(data, secData, callback);
//            } else {
//            	SettingIFAAFaceTypeActivity.this.runOnUiThread(new Runnable() {
//                    @Override
//                    public void run() {
//                        new FingerPrintToast(SettingIFAAFaceTypeActivity.this, FingerPrintToast.ST_REGFAIL).show("注册指纹失败");
//                        FingerprintBroadcastUtil.sendIdentifyStatusChangeMessage(SettingIFAAFaceTypeActivity.this, 0);
//                    }
//                });
//            }
//        }
...
//                public void run() {
//                    if (response.getResult() == AuthenticatorResponse.RESULT_SUCCESS) {
//                        new FingerPrintToast(SettingIFAAFaceTypeActivity.this, FingerPrintToast.ST_DEREGSUCCESS).show(null);
//                        saveToken("");
//                        isInited = false;
//                        handler.sendEmptyMessage(MSG_REGENABLE);
//                    } else {
//                        new FingerPrintToast(SettingIFAAFaceTypeActivity.this, FingerPrintToast.ST_DEREGFAIL).show("注销指纹失败");
//                    }
//                }
//            });
//        }
...
//                                new FingerPrintToast(SettingIFAAFaceTypeActivity.this, FingerPrintToast.ST_REGSUCCESS).show("");
//                                saveToken(IFAAFingerprintOpenAPI.getInstance().getToken());
//                                token = IFAAFingerprintOpenAPI.getInstance().getToken();
//                                isInited = false;
//                                handler.sendEmptyMessage(MSG_REGDISABLE);
//                            } else {
//                                new FingerPrintToast(SettingIFAAFaceTypeActivity.this, FingerPrintToast.ST_REGFAIL).show("ifaa注册指纹失败");
//                            }
//                            FingerprintBroadcastUtil.sendIdentifyStatusChangeMessage(SettingIFAAFaceTypeActivity.this, 0);
//                        }
//                    });
...
//    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ((TextView) findViewById(R.id.header_text)).setText("人脸免密");
        ...
        supportBIOTypes = EDISAuthManager.getSupportBIOTypes(this);

        mAccountDao = new AccountDao(SettingIFAAFaceTypeActivity.this);
        ...
        cancelScanButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                SettingIFAAFaceTypeActivity.this.finish();
            }
        });
        ...
        mRegActBtn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (m_log) {
					...
					intent.setClass(SettingIFAAFaceTypeActivity.this,com.sheca.finger.MainActivity.class);
    ...
    private void initData() {
        ...
        final String ifaaURL = SettingIFAAFaceTypeActivity.this.getString(R.string.UMSP_Service_IFAA);
...
//			Toast.makeText(SettingIFAAFaceTypeActivity.this, "检查注册状态初始化出错了:" + etasResult.getMsg(), Toast.LENGTH_SHORT).show();
//			return;
//		}
//
//		new MyAsycnTaks(){
...
//				UniTrust mUnitTrust = new UniTrust(SettingIFAAFaceTypeActivity.this, false);
...
//					checkIfaaStatus(ifaaBaseInfo);
//				}else{
//					//Toast.makeText(SettingIFAAFaceTypeActivity.this, "查询注册状态错误:"+resultStr+","+retMsg,Toast.LENGTH_SHORT).show();
//				}
//			}
//		}.execute();
//
...
//								// 服务器尚未注册或者出错
//								if (etasResult.getCode() != IfaaCommon.IFAAErrorCodeEnum.SUCCESS) {
//									Toast.makeText(SettingIFAAFaceTypeActivity.this, etasResult.getMsg() , Toast.LENGTH_SHORT).show();
...
//								if (etasResult.getCode() != IfaaCommon.IFAAErrorCodeEnum.SUCCESS) {
//									Toast.makeText(SettingIFAAFaceTypeActivity.this, "本地尚未注册/出错 ： " + etasResult.getMsg() , Toast.LENGTH_SHORT).show();
//									//tvShowInfos.append("本地尚未注册/出错 ： " + etasResult.getMsg() + "\n");
//									//ifaaSwitch.setChecked(false);
//									break;
//								}
//
//								Toast.makeText(SettingIFAAFaceTypeActivity.this, "IFAA 已经注册" , Toast.LENGTH_SHORT).show();
//								setIFAADisable(true);
                            ...
                            public void onResult(IFAAResult IFAAResult) {
                                ...
                                if (IFAACommon.IFAA_SUCCESS.equals(IFAAResult.getCode())) {
                                    ...
                                    runOnUiThread(new Runnable() {
                                        ...
                                        public void run() {
                                            if (IFAAAuthTypeEnum.AUTHTYPE_FINGERPRINT == ifaaBaseInfo.getAuthType()) {
                                                Toast.makeText(SettingIFAAFaceTypeActivity.this, "注册成功", Toast.LENGTH_SHORT).show();
                                                setIFAADisable(false);
                                    ...
                                    runOnUiThread(new Runnable() {
                                        ...
                                        public void run() {
                                            if (IFAAAuthTypeEnum.AUTHTYPE_FINGERPRINT == ifaaBaseInfo.getAuthType()) {
                                                MyLog.debug("指纹注册失败");
                                                Toast.makeText(SettingIFAAFaceTypeActivity.this, "注册失败：" + IFAAResult.getMsg(), Toast.LENGTH_SHORT).show();
                                                setIFAAEnable(true);
//                                        updateSwitch(false);
...
//		// 如果操作失败，辣么结束流程
//		if (etasResult.getCode() != IfaaCommon.IFAAErrorCodeEnum.SUCCESS) {
//			Toast.makeText(SettingIFAAFaceTypeActivity.this, "注册初始化失败 :" + etasResult.getMsg(), Toast.LENGTH_SHORT).show();
//			//tvShowInfos.append("注册初始化失败 ： " + etasResult.getMsg() + '\n');
//			if (etasResult.getCode() == IfaaCommon.IFAAErrorCodeEnum.STATUS_NOT_ENROLLED) {
//
//				//此时可引导用户录入指纹后在做注册操作
//				Toast.makeText(SettingIFAAFaceTypeActivity.this, "该手机未录入指纹或人脸" , Toast.LENGTH_SHORT).show();
//			}
//			return ;
//		}
//
...
//					reg(ifaaBaseInfo);
//				}else{
//					Toast.makeText(SettingIFAAFaceTypeActivity.this, "注册初始化失败:"+resultStr+","+retMsg,Toast.LENGTH_SHORT).show();
//				}
//			}
//		}.execute();
//
...
//							Toast.makeText(SettingIFAAFaceTypeActivity.this, "注册请求失败 ： " + e.getMessage(), Toast.LENGTH_SHORT).show();
...
//							} else {
//								//closeProgDlg();
//								Toast.makeText(SettingIFAAFaceTypeActivity.this, "注册失败 :" + etasResult.getMsg(), Toast.LENGTH_SHORT).show();
//								setIFAAEnable(true);
...
//                                        updateTextView("人脸注销失败,code: " + IFAAResult.getCode() + ",msg: " + IFAAResult.getMsg() + "\n");
                                                closeProgDlg();
                                                Toast.makeText(SettingIFAAFaceTypeActivity.this, "本地注销失败 ： " + IFAAResult.getMsg(), Toast.LENGTH_SHORT).show();
...
//		// 如果操作失败，辣么结束流程
//		if (etasResult.getCode() != IfaaCommon.IFAAErrorCodeEnum.SUCCESS) {
//			Toast.makeText(SettingIFAAFaceTypeActivity.this, "注销初始化失败 ： " + etasResult.getMsg(), Toast.LENGTH_SHORT).show();
...
//					unreg(ifaaBaseInfo);
//				}else{
//					Toast.makeText(SettingIFAAFaceTypeActivity.this, "注销初始化失败:"+resultStr+","+retMsg,Toast.LENGTH_SHORT).show();
//				}
//			}
//		}.execute();
//
...
//									if (etasResult.getCode() != IfaaCommon.IFAAErrorCodeEnum.SUCCESS) {
//										closeProgDlg();
//										Toast.makeText(SettingIFAAFaceTypeActivity.this, "本地注销失败 ： " + etasResult.getMsg() , Toast.LENGTH_SHORT).show();
...
//	}
    ...
    private void setIFAADisable(boolean isInit) {
        ...
        m_log = false;
        group_temo.check(R.id.radio1);
        DaoActivity.ifaaFaceAuth = true;
        editor.putBoolean(CommonConst.SETTINGS_IFAA_FACE_ENABLED, true);    //不使用指纹模块
        editor.commit();
    ...
    private void setIFAAEnable(boolean isInit) {
        closeProgDlg();
        Toast.makeText(SettingIFAAFaceTypeActivity.this, "注销成功", Toast.LENGTH_SHORT).show();
        Editor editor = sharedPrefs.edit();
        final String mUserName = sharedPrefs.getString(CommonConst.PARAM_USERNAME, "");
        ...
        DaoActivity.ifaaFaceAuth = false;
        editor.putBoolean(CommonConst.SETTINGS_IFAA_FACE_ENABLED, false);    //不使用指纹模块
        editor.commit();
...
//	        String info = AuthenticatorManager.getAuthData(SettingIFAAFaceTypeActivity.this, mAccountDao.getLoginAccount().getName());
//	        String deviceId = LaunchActivity.authenticator.getDeviceId();
//	        IFAAFingerprintOpenAPI.getInstance().getIFAARegRequestAsyn(SettingIFAAFaceTypeActivity.this,mAccountDao.getLoginAccount().getName(),info,deviceId,regFlag, callback);
...
//         String info = AuthenticatorManager.getAuthData(SettingIFAAFaceTypeActivity.this, mAccountDao.getLoginAccount().getName());
//         IFAAFingerprintOpenAPI.getInstance().getIFAADeregRequestAsyn(SettingIFAAFaceTypeActivity.this,mAccountDao.getLoginAccount().getName(),info, callback);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == FINGER_CODE) {
            if (resultCode == MainActivity.RESULT_OK) {
                new FingerPrintToast(SettingIFAAFaceTypeActivity.this, FingerPrintToast.ST_REGSUCCESS).show("input pwd");
            }
